...
def q(im): return im.convert('RGBA').quantize(colors=128,method=Image.Quantize.FASTOCTREE)
...
def shadow(size,extra=False):
 w,h=size; im=Image.new('RGBA',size,(0,0,0,0)); m=Image.new('L',size,0); d=ImageDraw.Draw(m); cx=w*.5
 d.ellipse((cx-48,50,cx+48,145),fill=210); d.polygon([(cx-55,130),(cx+58,130),(cx+85,h*.55),(cx+42,h*.64),(cx+25,h-30),(cx-10,h-22),(cx-20,h*.66),(cx-75,h*.56)],fill=210)
 if extra: d.polygon([(cx+42,h*.52),(w-18,h*.72),(w-34,h*.78),(cx+20,h*.62)],fill=185)
...
def pool(out):
 ...
 sp=Image.new('RGBA',(520,430),(0,0,0,0)); s=ImageDraw.Draw(sp); s.polygon([(55,330),(95,245),(145,285),(180,170),(220,260),(260,105),(305,255),(360,150),(390,280),(452,225),(475,340)],fill=(205,244,255,215)); s.ellipse((55,280,475,410),fill=(179,232,248,175))
 for cx,cy,r in [(155,115,20),(250,52,24),(370,88,18),(90,180,15),(430,155,13),(315,40,10)]: s.ellipse((cx-r,cy-r,cx+r,cy+r),fill=(225,250,255,235))
 save(q(sp.filter(ImageFilter.GaussianBlur(1.2))),out/'early-splash/splash.png')
...
def evidence(root,out):
 out.mkdir(parents=True,exist_ok=True)
 # machine-review contact sheets; never a human-quality PASS
 for scene in ['extra-shadow','early-splash','color-theft']:
  bg={'extra-shadow':'park.png','early-splash':'pool.png','color-theft':'table.png'}[scene]
  im=Image.open(root/scene/bg).convert('RGBA')
  if scene=='extra-shadow':
   for fn,xy in [('shadow-natural.png',(180,1030)),('shadow-natural.png',(470,1030)),('person-a.png',(280,710)),('person-b.png',(540,710)),('shadow-impossible.png',(615,1040))]: im.alpha_composite(Image.open(root/scene/fn).convert('RGBA'),xy)
  elif scene=='early-splash':
   im.alpha_composite(Image.open(root/scene/'ball.png').convert('RGBA'),(450,470)); im.alpha_composite(Image.open(root/scene/'splash.png').convert('RGBA'),(280,1065))
  else:
   im.alpha_composite(Image.open(root/scene/'vase-red.png').convert('RGBA'),(390,820)); im.alpha_composite(Image.open(root/scene/'ball-pale.png').convert('RGBA'),(745,1125))
  save(q(im),out/f'{scene}-post.png')

if __name__=='__main__':
 ap=argparse.ArgumentParser(); ap.add_argument('--output-root',default='factory-native/assets/resources/assets'); ap.add_argument('--evidence-dir',default='evidence/native-visual-v1'); a=ap.parse_args(); out=Path(a.output_root)
 park(out); save(q(person((208,112,64,255),(46,60,77,255),(58,39,29,255))),out/'extra-shadow/person-a.png'); save(q(person((63,137,162,255),(55,52,68,255),(38,30,26,255),True)),out/'extra-shadow/person-b.png'); save(q(shadow((360,520))),out/'extra-shadow/shadow-natural.png'); save(q(shadow((380,560),True)),out/'extra-shadow/shadow-impossible.png'); pool(out); table(out); evidence(out,Path(a.evidence_dir)); print('NATIVE_VISUAL_ASSETS_GENERATED')
